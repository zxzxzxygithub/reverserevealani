...
//		setBackgroundColor(mBgColor);
	...
	private void initTransform() {
		...
		float xSScale = mOriginalWidth / ((float) getWidth());
		float ySScale = mOriginalHeight / ((float) getHeight());
		...
		float xEScale = getWidth() / ((float) getWidth());
		float yEScale = getHeight() / ((float) getHeight());
		...
		float bitmapEndWidth = getWidth() * mTransfrom.endScale;// 图片最终的宽度
		float bitmapEndHeight = getHeight() * mTransfrom.endScale;// 图片最终的宽度
	...
	private void getCenterCropMatrix() {
		...
		/* 下面实现了CENTER_CROP的功能 */
		float xScale = mOriginalWidth / ((float) getWidth());
		float yScale = mOriginalHeight / ((float) getHeight());
		float scale = xScale > yScale ? xScale : yScale;
		mSmoothMatrix.reset();
		mSmoothMatrix.setScale(scale, scale);
		mSmoothMatrix.postTranslate(-(scale * getWidth() / 2 - mOriginalWidth / 2), -(scale * getHeight() / 2 - mOriginalHeight / 2));
	}
	...
	private void getBmpMatrix() {
		...
		mSmoothMatrix.postTranslate(-(mTransfrom.scale * getWidth() / 2 - mTransfrom.rect.width / 2),
				-(mTransfrom.scale * getHeight() / 2 - mTransfrom.rect.height / 2));
	}
